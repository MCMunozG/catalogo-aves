...
export class MySightingsPageComponent {
  ...
  readonly visibleSightings = computed(() =>
    this.mySightings().filter(
      (sighting) =>
        this.filter() === "ALL" ||
        (this.filter() === "DRAFT"
          ? sighting.status === "DRAFT"
          : sighting.status !== "DRAFT"),
    ),
  );
  readonly stats = computed(() => ({
    total: this.mySightings().length,
    drafts: this.mySightings().filter((sighting) => sighting.status === "DRAFT")
      .length,
    shared: this.mySightings().filter((sighting) => sighting.status !== "DRAFT")
      .length,
    mapVisible: this.mySightings().filter((sighting) =>
      this.isMapVisible(sighting),
    ).length,
  }));
  ...
  loadSightings(): void {
    ...
    this.observationsApi.mine().subscribe({
      next: ({ data }) => {
        this.mySightings.set(data);
        this.loading.set(false);
      },
      error: () => {
        this.error.set("No fue posible cargar tu bitácora.");
        this.loading.set(false);
      },
    });
  }
  ...
  edit(sighting: Sighting): void {
    ...
    if (!location) {
      this.error.set(
        "No fue posible recuperar la ubicación privada de este registro.",
      );
      return;
    }
    this.error.set("");
    ...
    this.editingId.set(sighting.id);
    this.draft = {
    ...
    this.selectedLocation.set({
      ...location,
      label: sighting.region || "Ubicación privada guardada",
    });
  }
  ...
  save(): void {
    ...
    if (!id || !location) {
      this.error.set("Selecciona una ubicación en el mapa antes de guardar.");
      return;
    }
    ...
    this.observationsApi
      ...
      .subscribe({
        next: () => {
          ...
          this.message.set("Avistamiento actualizado.");
          this.loadSightings();
        },
        error: () => {
          this.saving.set(false);
          this.error.set("No fue posible guardar el avistamiento.");
        },
      });
  }
  ...
  publish(sighting: Sighting): void {
    ...
    if (!location) {
      this.error.set(
        "No fue posible recuperar la ubicación privada para publicar este registro.",
      );
      return;
    }
    this.saving.set(true);
    ...
    this.observationsApi
      ...
      .subscribe({
        next: (updated) => {
          this.saving.set(false);
          this.message.set(
            this.isMapVisible(updated)
              ? "Avistamiento publicado y visible en el mapa."
              : "Avistamiento compartido con ubicación protegida; no se muestra un marcador público.",
          );
          this.loadSightings();
        },
        error: () => {
          this.saving.set(false);
          this.error.set(
            "No fue posible publicar. Intenta de nuevo cuando Catalog esté disponible.",
          );
        },
      });
  }
  ...
  delete(id: string): void {
    ...
    this.observationsApi.deleteSighting(id).subscribe({
      next: () => {
        this.saving.set(false);
        this.deletingId.set(null);
        this.message.set("Avistamiento eliminado.");
        this.loadSightings();
      },
      error: () => {
        this.saving.set(false);
        this.error.set("No fue posible eliminar el avistamiento.");
      },
    });
  }

  /** Abre el mapa público centrado en el registro ya autorizado para publicar. */
  viewOnMap(sighting: Sighting): void {
    this.router.navigate(["/mapa"], { queryParams: { sighting: sighting.id } });
  }
  ...
  speciesName(id?: string | null): string {
    return (
      this.species().find((species) => species.id === id)?.common_name ??
  ...
  statusLabel(status: string, sensitivity = "HIDDEN"): string {
    ...
    return sensitivity === "APPROXIMATE"
      ? "Publicado · zona aproximada"
      : "Publicado · punto autorizado";
  }
  /** Un registro compartido puede conservar su ubicación oculta si la especie lo requiere. */
  isMapVisible(sighting: Sighting): boolean {
    return (
      sighting.status !== "DRAFT" &&
      sighting.sensitivity !== "HIDDEN" &&
  ...
  private emptyDraft(): SightingDraft {
    return {
      species_id: "",
      observed_at: this.toLocalInput(new Date().toISOString()),
      individuals: 1,
      region: "",
      behavior: "",
      notes: "",

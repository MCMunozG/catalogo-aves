...
import { CommunityComment, FeedPost } from "../../../shared/models/community.models";

@Component({ standalone: true, imports: [FormsModule, RouterLink], templateUrl: "./community-feed.page.html", styleUrl: "./community-feed.page.scss" })
...
export class CommunityFeedPageComponent {
  ...
  readonly pagination = signal({ currentPage: 1, lastPage: 1, perPage: 8, total: 0 });
  ...
  constructor() {
    this.route.queryParamMap.subscribe((params) => {
      ...
      this.loadFeed(this.focusedPostId ? 1 : this.pagination().currentPage, this.focusedPostId);
    });
  }
  ...
  loadFeed(page = this.pagination().currentPage, postId?: string): void {
    ...
    this.api.feed(page, perPage, postId).subscribe({
      next: ({ data, current_page, last_page, per_page, total }) => {
        this.feed.set(data);
        this.pagination.set({ currentPage: current_page, lastPage: last_page, perPage: per_page, total });
  ...
  goToPage(page: number): void {
    ...
    if (this.focusedPostId) {
      ...
      this.router.navigate([], { relativeTo: this.route, queryParams: { post: null }, queryParamsHandling: "merge" });
      return;
    }
    this.loadFeed(page);
  }
  ...
  openComposer(): void { this.actionError.set(""); this.session.hasToken() ? this.composerOpen.set(true) : this.router.navigate(["/ingresar"], { queryParams: { returnUrl: "/comunidad" } }); }
  /** Publica una reflexión general, sin obligar a inventar una observación asociada. */
  publish(): void { const body = this.postBody.trim(); if (!body) return; this.api.createPost(body).subscribe({ next: () => { this.postBody = ""; this.composerOpen.set(false); this.loadFeed(); }, error: () => this.actionError.set("No fue posible publicar. Tu sesión puede haber expirado.") }); }
  ...
  like(postId: string): void {
    ...
    const request = liked ? this.api.removeReaction(postId) : this.api.react(postId);
    ...
    request.subscribe({
      next: ({ reacted, reactions_count }) => {
        ...
        this.feed.update((posts) => posts.map((post) => post.id === postId
  ...
  toggleComment(postId: string): void { if (!this.session.hasToken()) return this.openComposer(); const opening = this.commentingPostId() !== postId; this.commentingPostId.set(opening ? postId : null); this.commentBody = ""; if (opening) this.loadComments(postId); }
  /** Envía un comentario corto y cierra su formulario al completar. */
  sendComment(postId: string): void { const body = this.commentBody.trim(); if (!body) return; this.api.comment(postId, body).subscribe({ next: () => { this.commentBody = ""; this.loadComments(postId); this.loadFeed(); }, error: () => this.actionError.set("No fue posible enviar el comentario.") }); }
  /** Copia una URL estable de la publicación para compartirla sin depender de una red externa. */
  share(postId: string): void { navigator.clipboard?.writeText(`${location.origin}/comunidad#${postId}`).then(() => this.sharedPostId.set(postId)).catch(() => this.sharedPostId.set(postId)); }
  /** Recupera el hilo solo cuando se abre, para no descargar todos los comentarios del feed. */
  private loadComments(postId: string): void { this.api.comments(postId).subscribe({ next: (comments) => this.commentsByPost.update((current) => ({ ...current, [postId]: comments })), error: () => this.actionError.set("No fue posible cargar los comentarios.") }); }
}
